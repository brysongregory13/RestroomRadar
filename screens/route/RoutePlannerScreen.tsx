  ...
  <script>
    ...
    function initMap() {
      ...
      if (polylinePoints.length > 0) {
        ...
        new google.maps.Polyline({ path: polylinePoints, strokeColor: '#00897B', strokeOpacity: 0.8, strokeWeight: 3, map: map });
      }
      if (startLocation) new google.maps.Marker({ position: startLocation, map: map, icon: makeLetterIcon('A', '#43A047'), zIndex: 100 });
      if (endLocation) new google.maps.Marker({ position: endLocation, map: map, icon: makeLetterIcon('B', '#E53935'), zIndex: 100 });
      routeRestrooms.forEach(function(r) {
        var m = new google.maps.Marker({ position: { lat: r.lat, lng: r.lng }, map: map, icon: makePinIcon(r.index), zIndex: 50 });
        (function(id) { m.addListener('click', function() { window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'pin_tap', id: id })); }); })(r.id);
      });
    }
  </script>
...
function useAutocomplete(query: string, active: boolean) {
  const [predictions, setPredictions] = useState<PlacePrediction[]>([]);
  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    if (!query.trim() || !active) { setPredictions([]); return; }
  ...
  return { predictions, clear: () => setPredictions([]) };
}
...
export function RoutePlannerScreen({ navigation }: Props) {
  ...
  const originAC = useAutocomplete(origin, focusedField === 'origin');
  const destAC = useAutocomplete(destination, focusedField === 'destination');

  const handleMessage = useCallback((event: WebViewMessageEvent) => {
    try {
      const msg = JSON.parse(event.nativeEvent.data);
      if (msg.type === 'pin_tap') navigation.push('Detail', { id: msg.id });
    } catch {}
  }, [navigation]);
  ...
  async function handleSelectPrediction(field: Field, prediction: PlacePrediction) {
    if (field === 'origin') {
    ...
    setFocusedField(null);
    // Resolve to readable address
    const loc = await getPlaceLocation(prediction.placeId);
    if (loc) {
      if (field === 'origin') setOrigin(loc.formattedAddress);
      else setDestination(loc.formattedAddress);
    }
  }
  ...
  const showOriginDropdown = focusedField === 'origin' && originAC.predictions.length > 0;
  const showDestDropdown = focusedField === 'destination' && destAC.predictions.length > 0;
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView keyboardShouldPersistTaps="always">
        <View style={styles.inputCard}>
          ...
          <TextInput
            style={[styles.input, originError && styles.inputError]}
            value={origin}
            onChangeText={(t) => { setOrigin(t); setOriginError(false); }}
            onFocus={() => setFocusedField('origin')}
            onBlur={() => setTimeout(() => setFocusedField((f) => (f === 'origin' ? null : f)), 150)}
          ...
          {showOriginDropdown && (
            <FlatList
              ...
              data={originAC.predictions}
              keyExtractor={(p) => p.placeId}
              keyboardShouldPersistTaps="always"
              scrollEnabled={false}
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.dropdownItem} onPress={() => handleSelectPrediction('origin', item)}>
          ...
          <TextInput
            style={[styles.input, destError && styles.inputError]}
            value={destination}
            onChangeText={(t) => { setDestination(t); setDestError(false); }}
          ...
          {showDestDropdown && (
            <FlatList
              style={styles.dropdown}
              data={destAC.predictions}
              keyExtractor={(p) => p.placeId}
              keyboardShouldPersistTaps="always"
              scrollEnabled={false}
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.dropdownItem} onPress={() => handleSelectPrediction('destination', item)}>
                  <Text style={styles.dropdownIcon}>📍</Text>
                  <Text style={styles.dropdownText} numberOfLines={2}>{item.description}</Text>
                </TouchableOpacity>
              )}
          ...
          <TouchableOpacity style={styles.searchBtn} onPress={handleSearch} disabled={loading}>
            {loading ? (
              ...
              <Text style={styles.searchBtnText}>Find Restrooms Along Route</Text>
            )}
          </TouchableOpacity>
        </View>
              ...
              return (
                ...
                >
                  ...
                  <View style={styles.resultInfo}>
                    ...
                    <Text style={[styles.resultStatus, { color: open ? Colors.success : Colors.danger }]}>
                      {open ? 'Open' : 'Closed'}
                    </Text>
...
const styles = StyleSheet.create({
  ...
  dropdownIcon: { fontSize: 13, marginRight: Theme.spacing.sm },
  dropdownText: { flex: 1, fontSize: Theme.typography.fontSizeSM, color: Colors.textPrimary },
  searchBtn: {
